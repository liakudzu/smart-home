...
use smart_home::{Report, SmartDevice, SmartSocket, SmartThermometer};
...
fn main() {
    ...
    room.register_callback(|name, device| {
        println!(
            "[Callback] Добавлено новое устройство: '{}' - {}",
            name,
            device.report()
        );
    });
    ...
    room.add_device(
        ...
        SmartThermometer::new(23.0).into(),
    );
}
